...
from csb.bio.hmm import State, Transition, ProfileHMM, HMMLayer, StateExistsError, \
                        ProfileLength, HHpredHitList, HHpredHit, ScoreUnits, States
...
class HHpredProfileParser(object):
    ...
    Produces a L{ProfileHMM} object representation of a given HHpred profile HMM.    
    
    @param hhm_file: *.hhm file name to parse
    @type hhm_file: str
    @param pdb_file: an optional hhm structure file, containing the structural data,
                    associated with the HMM.
    @type pdb_file: str 
    @note: This is NOT a regular PDB file! It must be specifically formatted for use
           with HHpred.
    ...
    def format_structure(self, input_pdb, chain_id, output_file):
        """
        Format input_pdb as a HHpred pre-parsed structure file for the current HMM.
        Formatting means: only chain_id left in the PDB file, residue sequence numbers 
        strictly corresponding to the HMM states' ranks. 
        
        @param input_pdb: source PDB file name
        @type input_pdb: str
        @param chain_id: source chain ID (which chain to pick from the structure)
        @type chain_id: str
        @param output_file: save the output PDB file here
        @type output_file: str
        
        @raise ValuError: if the specified source chain has a different length or residues, 
                          compared to the profile HMM; or when a residue's rank is out of range
                          again with reference to the HMM 
        """
        
        hmm = self.parse()
        s = csb.bio.io.pdb.StructureParser(input_pdb).parse_structure()
        chain = s.chains[chain_id]
        
        if hmm.layers.length != chain.length:
            raise ValueError('Chain {0.id} ({0.length} residues) does not have the expected length ({1})'.format(chain, hmm.layers.length))
            ...
            try:
                if hmm.layers[residue.rank].residue.type != residue.type:
                    if residue.type == 'X':
                        ...
                        raise ValueError('Residue {0} differs from the residue under the layer with the same rank: {1}'.format(
                                residue, 
                                hmm.layers[residue.rank].residue.type))
            except csb.pyutils.ItemNotFoundError:
                raise ValueError("Residue {0} is out of the HMM's range {1}..{2}".format(residue, hmm.layers.start_index, hmm.layers.last_index))                 
    ...
    def parse(self, units=ScoreUnits.LogScales):
        ...
        @param units: also convert the profile score units to the specified L{ScoreUnits} kind
        ...
        if self._sequences:
            ...
            raise InvalidHHpredFormatError('No profile MSA and secondary structure found.')
    ...
    def _chop(self):
        """
        Chop the HHM file into pieces - HMM properties, secondary structure + MSA, HMM.
        """
        ...
        try:
            ...
            raise InvalidHHpredFormatError('This file is either not in HHM format, or it is corrupt.')  
    ...
    def _parse_properties(self, hmm):
        ...
        for line in self._properties:
            ...
            if line.startswith('NAME '):
                ...
                raise NotImplementedError('Unexpected line. Don\'t know how to parse field {0}.'.format(line[0:5]))  
    ...
    def _parse_profile(self, hmm, units=ScoreUnits.LogScales):
        ...
        @raise NotImplementedError: when an unknown transition string is encountered             
        """
        assert self._chopped
        ...
        if units == ScoreUnits.LogScales:
            ...
            def parse_probability(v):
                if v.strip() == '*':
                    ...
                    return hmm._convert(units, float(v), hmm.scale, hmm.logbase)            
        ...
        while True:
            ...
            if line.startswith('NULL'):
                try:
                    ...
                    residues = [ csb.pyutils.Enum.parse(sequence.SequenceAlphabets.Protein, aa) for aa in residues ]
                ...
                rank = int(emprobs[1])
                residue = structure.ProteinResidue(rank=rank, type=emprobs[0], sequence_number=rank, insertion_code=None)
                
                new_layer = hmm.layers.append(HMMLayer(rank, residue))
                assert new_layer == rank, '{0} vs {1}'.format(rank, new_layer)
                
                match = State(States.Match, emit=csb.pyutils.Enum.members(sequence.SequenceAlphabets.Protein))                
                match.rank = rank
        ...
        # 2. Append starting transitions: S -> M[1] and optionally S -> D[1]. In the latter case D[1] 
        #    will be created and attached to first layer beforehand.                 
        if len(hmm.layers) > 0:
        ...
        # 3. Append remaining transitions. If a transition is demanding an I or D state, create it on the fly
        #    according to the HMM graph model. For such reasons the main order of transition parsing is significant:   
        #        1) M[i]->any(M[i+1], I[i], D[i+1]) - this creates the I[i] and D[i+1] states coming from M[i] 
        #        2) I[i]->any(I[i], M[i+1]) and D[i]->any(M[i+1], D[i+1]) - this creates D[i+1] coming from D[i] 
        for rank, fields in enumerate(tran_lines, start=hmm.layers.start_index):
            ...
            for col, tran in enumerate(tran_types):
                ...
                if tran == 'M->M':                    
                    ...
                    insertion = State(States.Insertion, emit=csb.pyutils.Enum.members(sequence.SequenceAlphabets.Protein))
    ...
    def _parse_structure(self, hmm):
        ...
        @raise InvalidHHpredFormatError: when any residue cannot be assigned to layer from the HMM
        @raise NotImplementedError: when an unknown data field is encountered in the PDB file           
        """
            ...
            for line in pdb:
                ...
                if line.startswith('HEADER'):
                    ...
                    try:
                        ...
                        raise InvalidHHpredFormatError('ProteinResidue {0} at fixed position {1} (former {2}) is out of range.'.format(line[17:20], rank + offset_fix, rank))
                    
                    serial_number = int(line[6:11])
                    name = line[12:16]
                    element = line[76:78].strip()
                    try:
                        element = csb.pyutils.Enum.parsename(structure.ChemElements, element)
        ...
        for layer in hmm.layers:
            ...
            layer.residue.torsion = layer.residue.compute_torsion(prev_residue, next_residue, strict=False)                
...
class HHpredOutputParser(object):
    ...
    @param alignments: if set to False, the parser will not read the alignments 
                       section in the file at all 
            ...
            if in_hits:
                ...
                if line.startswith('Done'):
                    ...
                    if c_rank not in hits:
                        raise InvalidHHpredOutputError('Alignment {0}. refers to a non-existing hit'.format(c_rank))
                    ...
                    for pair in line.split():
                        ...
                        if key == 'Identities':
                            hits[c_rank].identity = float(value.replace('%', ''))
                        elif key == 'Similarity':
                            hits[c_rank].similarity = float(value)
                        elif key == 'Sum_probs':
                            hits[c_rank].prob_sum = float(value)
                elif line.startswith('Q ') and not line[:11].rstrip() in ('Q Consensus', 'Q ss_pred', 'Q ss_dssp'):
                    for residue in line[22:]:
                        if residue.isspace() or residue.isdigit():
                            ...
                            has_alis = True
                elif line.startswith('T ') and not line[:11].rstrip() in ('T Consensus', 'T ss_pred', 'T ss_dssp'):
                    for residue in line[22:]:
                        if residue.isspace() or residue.isdigit():
                            break
                        else:
        ...
        if self.alignments and has_alis:
            for rank in alis:
                try:
                    ...
                    raise InvalidHHpredOutputError('Corrupt alignment at hit No {0}.\n {1}'.format(rank, er))
