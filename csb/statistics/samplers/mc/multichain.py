...
What follows is a short piece of code which will explain how to sample from a PDF using the replica
exchange (RE) method and the replica exchange with nonequilibrium switches (RENS) method.
It draws 5000 samples from a 1D normal distribution using the RENS algorithm working on three Markov
chains being generated by the HMC algorithm:
...
from csb.statistics.samplers.mc import ThermostattedMDRENSSwapParameterInfo, \
                                       AlternatingAdjacentSwapper
...
params = [ThermostattedMDRENSSwapParameterInfo(samplers[0], samplers[1], rens_timesteps[0], \
          ...
          ThermostattedMDRENSSwapParameterInfo(samplers[0], samplers[1], rens_timesteps[0], \
          rens_trajectory_length, rens_gradients[0])]
...
plt.hist([numpy.random.normal(size=5000), rawstates], bins=30, normed=True, \
         label=['numpy.random.normal', 'RENS + HMC'])
plt.legend()
plt.show()


For Replica Exchange (RE), the procedure is easier because apart from the two sampler instances
the corresponding RESwapParameterInfo objects take no arguments.

Every replica exchange algorithm in this module (ReplicaExchangeMC, MDRENS, ThermostattedMDRENS)
is used in a similar way. A simulation is always initialized with a list of samplers (instances
of classes derived from AbstractSingleChainMC) and a list of SwapParameterInfo objects suited for
the algorithm under consideration. Every SwapParameterInfo object holds all the information needed
to perform a swap between two samplers. The usual scheme is to swap only adjacent replicae in a
scheme
1 <--> 2, 3 <--> 4, ...
2 <--> 3, 4 <--> 5, ...
This swapping scheme is implemented in the AlternatingAdjacentSwapScheme class, but different schemes
can be easily implemented by deriving from AbstractSwapScheme.
Then the simulation is run by looping over the number of samples to be drawn and calling the sample()
method of the algorithm. By calling the swap_all() method of the specific AbstractSwapScheme
implementation, all swaps defined in the list of SwapParameterInfo objects are performed according
to the swapping scheme. The swap_all() method may be called for example after sampling intervals of a
fixed length or randomly.
"""
